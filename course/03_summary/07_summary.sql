...
SELECT COUNT(*) AS NumberOfRows,
       COUNT(DISTINCT CustomerId) AS NumberOfCustomers,
	   COUNT(DISTINCT CustomerId) / (COUNT(CustomerId) * 1.0) AS PcntOfUniqueCustomers
FROM "Order";
...
SELECT OrderDate FROM "Order" ORDER BY OrderDate LIMIT 1;
-- Wyznacz najnowszą datę zamówienia (kolumna OrderDate) z tabeli Order.
SELECT MAX(OrderDate) FROM "Order";
SELECT OrderDate FROM "Order" ORDER BY OrderDate DESC LIMIT 1;
